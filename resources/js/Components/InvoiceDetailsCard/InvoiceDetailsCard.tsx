      ...
      {data ? (
        <Stack>
          <Flex gap="sm" justify="flex-end">
            <Button leftSection={<IconCloudDownload size={ICON_SIZE} />} variant="light">
              Download
            </Button>
            <Button leftSection={<IconPrinter size={ICON_SIZE} />} variant="light">
              Print
            </Button>
            <Menu shadow="md" width={200}>
              <Menu.Target>
                <Button leftSection={<IconShare size={ICON_SIZE} />} variant="light">
                  Share
                </Button>
              </Menu.Target>
              ...
              <Menu.Dropdown>
                ...
                <Menu.Item leftSection={<IconSend size={14} />}>Forward in chat</Menu.Item>
              </Menu.Dropdown>
            </Menu>
          </Flex>
          <Text {...TEXT_PROPS}>Hello {data.full_name},</Text>
          <Text {...TEXT_PROPS}>
            This is the invoice for a payment of ${data.amount} you made to {data.client_company}
          </Text>
          <Flex
          ...
          >
            ...
            <Stack gap={4} style={{ textAlign: tablet_match ? 'start' : 'end' }}>
              <Text {...TEXT_PROPS} fw={600}>
                Payment Date
              </Text>
          ...
          >
            ...
            <Stack gap={4} style={{ textAlign: tablet_match ? 'start' : 'end' }}>
              ...
              <Text component="a" href={`mailto:${data.email}`} {...TEXT_PROPS} {...LINK_PROPS}>
                {data.email}
              </Text>
            </Stack>
          </Flex>
          ...
          <Text ta="center" fz="sm" c="dimmed" mt="md">
            Extra note: Please send all items at the same time to the shipping address. Thanks in
            advance.
          </Text>
        </Stack>
      ) : (
